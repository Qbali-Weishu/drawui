...
<template>
	...
	>
		...
		<teleport to="body">
			...
			<div class="createDomOut" v-if="showCreateBord" v-show="!showEventEdit" :style="{left :`${(bordLeft+deltaX)}px`,height:`${parent==''?'200px':'700px'}`}" ref="mainbord">
				...
				<div class="headerItem">
					...
					<inputhook :callIndex="0" :xpath="xpath" @dropHook="dropChangeTarget" style="left: 90%;margin-top: 2px;position: absolute;right: 0;top: 10px;"></inputhook>
				</div>
				<!--元素属性-->
				<div class="headerItem" v-if="parent!=''">
				...
				<!--属性编辑框-->
				<div class="mainArea" style="border: 2px solid white;"  v-if="parent!=''">
					<!--属性对-->
				...
				<!--样式预览-->
				<div  v-if="parent!=''" class="mainArea" style="overflow: hidden;background-color: var(--mainColorGray3);box-shadow: none;flex-shrink: 0;display: flex;justify-content: center;align-items: center;">
				...
				<!--样式编辑-->
				<div  v-if="parent!=''" class="mainArea" style="margin-top: 15px;height: 250px;">
					<!--引用类列表-->
				...
				<!--元素事件-->
				<div  v-if="parent!=''" class="headerItem">
				...
				<!--事件选择-->
				<div  v-if="parent!=''" class="mainArea" style="margin-bottom: 20px;">
		...
		<teleport to="#actionEdit" v-if="showEventEdit">
			<actionEdit v-bind="editVbind" ref="editThisEvent" style="left:0;top:40px;z-index: 999;height:calc(80vh - 40px);box-shadow: none;border-radius: 10px;" @quit="showEventEdit=false;" v-show="!showEventList"></actionEdit>
		</teleport>
	</div>
</template>
	...
	export default{
				...
				comDesc:"",
					...
					rightBarMenu:{//右键列表
						...
						options:[
								...
								{name:"复制",event:()=>{
									let index=this.logicIndex[this.rightBarMenu.selected].index
									this.$emit("copy",this.xpath+"=>"+this.logicIndex[index].xpath)
								}},
								{name:"删除",event:(item)=>{
									 let index=this.logicIndex[this.rightBarMenu.selected].index
					 ...
					 callTarget:{},
					  ...
					  newChildrenNodes:{},//当前生成元素的子元素列表，由子元素提供
					  newChildrenNodesIndex:[],//当前生成元素的子元素先后顺序，由子元素提供
					  newChildrenNodesXpath:[],
		...
		mounted(){
		    ...
		    if(this.isNew){
				...
				this.setContentTarget=['thisDomKey']
			...
			System.pushComDesc(this.thisECName,"/*修改元素*/\n"+this.parent)
			this.mountedAction()
		},
		methods:{
			...
			//自定义函数
			//接收修改对象
			dropChangeTarget:function(index,backContent="",varPath){
				let data=System.eventComponent_Out()
					if(System.requestCheckVarArea(varPath,this.xpath)){
						let target=data.change//调用系统事件中由锚点抛出源赋值的输入事件获取输出值
						let targetXpath=parent.xpath
						...
						if(Object.prototype.toString.call(data)=='[object Object]'){
							...
							try{
								if(data.node=="var"){
									if(this.parent!=("var:"+data.varName) && this.cloneNode!=null){
										System.showToast("目标变量名已修改",2000)
									}
									...
									if(this.cloneNode==null){
										this.domContent=""
										//若原来已有cloneNode，则不需要修改
										this.cloneNode=`<div style="width:200px;height:100px;background-color:#D9D9D9;flex-shrink:0" id=${this.getEcId()}></div>`
										System.showToast("已创建预览节点",2000)
										this.style=
											{
												'height':"100px",
												'width':"200px",
										...
										this.childrenNodes=[]
										this.childrenNodesIndex=[]
										
										
										this.classList=[]
										this.childrenNodesXpath=[]
									}
									this.setContentTarget=['domContent','cloneNode','parentNodeXPath','parent','parentCreateDom','childrenNodes','childrenNodesIndex','style','classList','childrenNodesXpath']
									this.setLogicContent()
									//挂载重置事件
									this.mountToWindow()
									this.$nextTick(()=>{
										...
										try{
											...
											this.editVbind.sourceXpath=this.xpath+".userDefine.domevent"
											System.eventComponent_Out=null
										}catch(e){
											console.error(e)
										}
									})
									...
									this.parentCreateDom=true
									this.parent=target.parentID
									this.cloneNode=target.cloneNode
									this.parentNodeXPath=target.parentPositon
									this.childrenNodes=target.childrenNodes
									this.childrenNodesIndex=target.childrenNodesIndex
									this.style=target.style
									this.classList=target.classList
									this.domContent=target.domContent
									//this.attr=target.attr
									this.childrenNodesXpath=target.childrenNodesXpath
									this.setContentTarget=['domContent','cloneNode','parentNodeXPath','parent','parentCreateDom','childrenNodes','childrenNodesIndex','style','classList','childrenNodesXpath']
									this.setLogicContent()
									//挂载重置事件
									this.mountToWindow()
					...
					alert('定义顺序混乱，请在修改对象之后使用此节点')
				}
			},
			//定义挂载完成后的执行时间
			mountToWindow:function(){
				window['resetThis'+this.parent+"_change"]=(varPath)=>{
					this.dropChangeTarget(0,"",varPath)
				}
				//被修改元素删除时，需要重置当前
				window['removeThis'+this.parent+"_change"]=()=>{
					...
					this.$refs['showCreatedDom'].innerHTML=""
					this.setContentTarget=['cloneNode','parentNodeXPath','parent','parentCreateDom','childrenNodes','childrenNodesIndex','style','classList','childrenNodesXpath']
					this.setLogicContent()
					window['removeThis'+this.parent+"_change"]=null
				}
			},
			close:function(){
				//同步样式
			...
			defineDom:function(tag,input){
				...
				//this.$refs['showCreatedDom'].appendChild(dom)
				//console.log(this.$refs['showCreatedDom'])
			...
			processCssText:function(){
				...
				for(let key in this.style){
					...
					if(reg.test(text))
						text=text.replace(reg,'-'+text.match(reg)[0].toLowerCase())
					//替换var /gradient等
			...
			insertToThis:function(node,childOwnXpath){
				...
				this.newChildrenNodesIndex.push(childOwnXpath)
				this.newChildrenNodes[childOwnXpath]=this.domToStr(node)
				if(this.showCreateBord){
					//编辑面板打开的情况下
					this.$nextTick(()=>{
						this.$refs['preView'].childNodes[0].appendChild(node)
				...
				this.$refs['showCreatedDom'].innerHTML=this.$refs['preView'].innerHTML
				this.setContentTarget=['childrenNodes','childrenNodesIndex','childNodesXpath','newChildrenNodesIndex','newChildrenNodes']
				this.setLogicContent()
			},
			insertChildNode:function(index,backContent="",varPath){
				...
				let data=System.eventComponent_Out()
				if(System.requestCheckVarArea(varPath,this.xpath)){
					if(varPath!=this.parentNodeXPath){
						...
						if(Object.prototype.toString.call(data)=='[object Object]'){
							...
							try{
								if(data.node=="var"){
									...
									console.log("添加新子节点",this.insertToThis,this.getPosition(),this.getEcId())
						...
						alert("引入错误,可能存在同时将节点用作同一父元素与子元素的情况")
					}
				}else{
					alert('定义顺序混乱，请将子组件上移至当前组件上方')
				}
