<template>
	<div 
	        draggable="false"
	        style="margin-top: 20px;min-width: 400px;height:165px;background-color: white;border-radius: 15px;padding: 10px;box-sizing: border-box;position: absolute;transition: .2s;" 
	        ref="forOut"
			:id="xpathId"
	        ...
	        >
	                <div class="header" draggable="false">
						<div class="logicName" >
	                        ...
	                        <!--头部参数列表-->
							<h3 style="margin: 0;padding: 0;font-weight: normal;margin-top: 10px;margin-left: 10px;line-height: 30px;">定时间隔</h3>
							<h4 style="margin: 0;padding: 0;margin-top: 10px;margin-left: 10px;font-weight: normal;width: 20px;height: 35px;line-height: 35px;background-color: transparent;color: var(--mainColorGray2);user-select: none;">每</h4>
							<div class="inputDiv" style="display: flex;box-sizing: border-box;overflow: hidden;width: 25%;margin-left: 0;">
								<div style="text-align: center;white-space: nowrap;width: 100%;overflow: hidden;text-overflow: ellipsis;height: 30px;outline: none;border: none;;line-height: 30px;letter-spacing: 0px;padding-left: 2px;box-sizing: border-box;"  contenteditable="true" @input.stop="inputTimeout"  @keydown="">{{timeoutValue}}</div>
								
							</div>
							<h4 style="margin: 0;padding: 0;margin-top: 10px;margin-left: 10px;font-weight: normal;width: 90px;height: 35px;line-height: 35px;background-color: transparent;color: var(--mainColorGray2);user-select: none;">毫秒执行一次</h4>
							<div class="headerText" style="margin: 0;background-color:var(--mainColorGray3);color: var(--mainColorBlue3);cursor: pointer;height: 30px;margin-top: 10px;line-height: 30px;border-radius: 10px;width: 80px;text-align: center;" @click="callSelectObject">{{['( )=>','function( )'][object]}}</div>
							<selectListT @change="selectChange_object" ref="selectList_object" v-if="showSelectObject" :list="['( )=>','function( )']" :selected="['( )=>','function( )'][object]" @close="showSelectObject=false" style="margin: 0;background-color: rgba(0,0,0,0.85);color: white;font-weight: normal;position: absolute;left: 175px;top: 5px;z-index: 153;" :width="'80px'"></selectListT>
					</div>
	...
	export default{
		...
		data(){
			return{
				...
				thisECName:"interval",//当前组件名称
				mountedIndex:0,//多个锚点的目标标记
				mounted:[false],//标记是否已经有锚点
		...
		mounted(){
		    ...
		    if(this.isNew){
				...
				this.setContentTarget=['timeoutValue','object']
				this.setLogicContent(true)
		    }else{
		       this.processLogic()
		    }
			System.pushComDesc(this.getEcId(),"/*定时执行:"+this.timeoutValue+"毫秒执行一次*/")
		},
		methods:{
			...
			inputTimeout:function(e){
				if(this.timeout!=null){
					clearTimeout(this.timeout)
				}
				this.timeout=setTimeout(()=>{
					this.timeoutValue=e.target.innerText
					this.setContentTarget=['timeoutValue','object']
