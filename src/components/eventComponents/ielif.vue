	...
	<div 
	        ...
	        >
	                <div class="header" draggable="false">
						...
						<h3 style="margin: 0;padding: 0;font-weight: normal;margin-top: 10px;margin-left: 10px;line-height: 30px;">条件判断</h3>
	                        <!--头部参数列表-->
	                </div>
	                ...
	                <div class="ecblock1 main" style="padding: 10px;box-sizing: border-box;position: relative;background-color: transparent;min-height: 50px;" ref="ecMain"  draggable="false">
	                        ...
	                        <div class="areaContentLine" >
	                                <!--顺序控制器-->
	                                <div class="logicViews" style="overflow-y: scroll;overflow-x: visible;">
	                                        <div class="logicIndexItem" style="overflow: visible;margin-top: 5px;" v-for="(indexItem,index) in logicIndex" :key="index" @contextmenu.stop="logicViewMenu($event,index)" @click="gotoArea(indexItem.index)">
	                                               <div style="width: 100%;overflow: hidden;padding-left: 15px;box-sizing: border-box;">{{indexItem.name}}</div> 
												<insertEC 
												:positionIndex="index+1" 
												@startInsertEC="startInsertEC" 
												style="width: 100px;margin-top: 0;margin-left: 5px;" 
												v-if="blocks.blockIndex[index]!='ifelseBlock'"></insertEC>
	                                        </div>
	                                </div>
	                        </div>
	                        ...
	                        <div @click.stop="startInsertEC($event,0)" draggable="false" @mousemove="null" @mousedown.stop="" @mouseup="" v-if="logicIndex.length==0" 
					...
					<!--组件特有，自插入-->
					<teleport to="body" v-if="blocks.show">
						<div style="position: absolute;width: 100px;height: 105px;z-index: 99;background-color:rgba(255,255,255,0.5);backdrop-filter: blur(5px);box-shadow: 0 0 15px rgba(0,0,0,0.15);border-radius: 10px;overflow: hidden;" :style="{top:`${blocks.top}px`,left:`${blocks.left}px`}">
							<input ref="closeInsert" @blur="closeInsert" style="position: absolute;left: -10px;height: 1px;width: 1px;padding: 0;outline: none;border: none;"/>
							<div class="ifItem" v-for="(item,index) in blocks.blockList" style="margin-top: 5px;" @click.stop="insertNewBlock(item.name)">{{item.text}}</div>
						</div>
					</teleport>
	        </div>
</template>
<script>
	import ifBlock from "./if.vue"
	import ifelseBlock from "./ifelse.vue"
	import ifelseifBlock from './ifelseif.vue'
	import insertEC from "./tools/insertEC.vue"
	...
	export default{
				...
				rightBarMenu:{//右键列表
				 ...
				 callTarget:{},
				  ...
				  blocks:{
					  ...
					  index:0,
					  blockList:[
							{name:"ifBlock",text:"if"},{name:'ifelseifBlock',text:"else if"},{name:'ifelseBlock',text:"else"}
					  ],
					  blockIndex:[],
					  if:0,
					  else:0
		...
		mounted(){
			...
			System.pushComDesc(this.getEcId(),"","")
		},
		methods:{
				...
				*/
			    ...
			    if(logicIndex==null){
			        ...
			        for(let i=0;i<this.childHeight.length-dif;i++){
						...
						//插入框位置
						//parentChilds[i+2].childNodes[1].style.top=185+"px"
			...
			startInsertEC:function(e,index){
				this.blocks.left=e.clientX
				this.blocks.top=e.clientY
				this.blocks.index=index
				console.log(index)
				this.blocks.show=true
				this.$nextTick(()=>{
					this.$refs['closeInsert'].focus()
				})
			},
			closeInsert:function(){
				setTimeout(()=>{
					this.blocks.show=false
				},200)
			},
			insertNewBlock:function(component){
				//骗过系统
				System.requestEcCallBack=true
				let i=""
				if(component=="ifBlock"){
					i="if"
					if(this.blocks.if>0){
						alert("if已存在")
						return
				...
				if(component=="ifelseBlock"){
					i="else"
					if(this.blocks.if==0){
						alert("if结构未插入")
						return
					}else{
						if(this.blocks.else>0){
							alert("else已存在")
							return
						}else{
							if(this.blocks.index!=this.requestConponent.length){
								alert("此处不允许插入else结构")
								return
							}else{
								this.blocks.else=1
				...
				this.childHeight.push(0)
				//像当前组件的逻辑索引插入新组件
				this.blocks.blockIndex.push(component)
				this.logicIndex.push({name:i,
				                      index:this.requestConponent.length,
				                      xpath:newXpath})
				...
				const finishEC=()=>{
				    ...
				    this.$nextTick(()=>{
				        ...
				        setTimeout(()=>{
							...
							let preLogicIndex=JSON.parse(JSON.stringify(this.logicIndex))
							let from=preLogicIndex.length-1
							let to=this.blocks.index
							let llg=preLogicIndex[from]
							//console.log(lg,from)
							//逻辑上移
							/*
							...
							//删除原位置
							this.logicIndex.splice(from+1,1)
							this.blocks.blockIndex.splice(t,0,component)
							//删除原位置
							this.blocks.blockIndex.splice(from+1,1)
							this.$nextTick(()=>{
							    this.setChildHeight(-1,0,false,
							                        {indexs:preLogicIndex,from:from,to:to})
							})
