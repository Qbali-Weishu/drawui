	        ...
	        >
	                <div class="header" draggable="false">
						<div class="logicName" >
	                        ...
	                        <!--头部参数列表-->
							<h3 style="margin: 0;padding: 0;font-weight: normal;margin-top: 10px;margin-left: 10px;line-height: 30px;">循环条件</h3>
							<div class="inputDiv" style="display: flex;box-sizing: border-box;overflow: hidden;">
								...
								<div style="text-align: center;white-space: nowrap;width: 100%;overflow: hidden;text-overflow: ellipsis;height: 30px;outline: none;border: none;;line-height: 30px;letter-spacing: 0px;padding-left: 2px;box-sizing: border-box;"  contenteditable="true" @input.stop="inputCondition"  @keydown="mountedIndex=0;deleteContent($event)">{{condition}}</div>
...
<script>
	...
	export default{
		...
		data(){
			return{
				comDesc:"while循环",//组件描述
				ecLogicName:this.logicName,//此逻辑命名
				thisECName:"while",//当前组件名称
		...
		mounted(){
		    ...
		    if(this.isNew){
				...
				System.pushComDesc(this.getEcId(),"/*while循环 :while("+this.condition+")*/")
