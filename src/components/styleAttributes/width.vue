...
<template>
    <div id="widthEdit">
        <pre style="color: var(--mainTextColor1);">宽  度</pre>
        <div style="position:relative;margin-right:20px;float:right;width:50%;height:30px;background-color:transparent;border-radius:10px;"><!--border: 2px solid #DFDFDF;-->
            <input @input="input" id="drawUISetWidth"  v-model="realData"/>
            <unit style="position:absolute;right:0;top:7px;" @unitChange="unitChange" @littleSet="littleSet" :defaultUnit="thisUnit"></unit>
            <button class="styleBindVar" style=";position:absolute;right:-20px;top:0" :data-attribute="bindName" @click="requestVar">{ }</button>
        </div>
	</div>
</template>
<style scoped>
...
export default{
    ...
    methods:{
		emitProcess:function(){
			let callBack=()=>{
				this.set(false)
				this.$emit("preProcess",'width',this.childrenData_edit)
			}
			this.$emit("emitPreFun",'width',callBack)
		},
        set:function(emit=true){
			...
			if(emit)
				this.$emit("set","width",this.childrenData_edit,()=>{return true},this.setted)
			this.setted=false
        },
        ...
        requestVar:function(e){//请求绑定变量
			this.$emit("requestVar",e,"width",this.realDataContion)
        },
